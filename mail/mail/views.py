...
def compose(request):
    ...
    for user in users:
        email = Email(
            ...
            subject=subject,
            body=body,
            read=user == request.user #email for the sender must have been already read
        )
        email.save()
        
        #from usernames of the recipients list 
...
serialize    return JsonResponse([email.() for email in emails], safe=False)
